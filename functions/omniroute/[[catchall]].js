// Cloudflare Pages Load Balancer Function for Omniroute
// Unified URL under project 'la-inconclusa'
...
export async function onRequest(context) {
  ...
  // Path after /omniroute
  const targetPath = '/' + (params.catchall ? params.catchall.join('/') : '');
  const searchParams = url.search;

  // Round-robin selection based on random index or failover
  ...
  try {
    ...
    // Failover request if primary threw network error
    targetUrl = `${BACKENDS[secondaryIndex]}${targetPath}${searchParams}`;
